...
- **A background caption is white, bold and sans serif.** The lettering is the one part of a background title that is not a style decision: white is what stays legible over footage that changes shade from shot to shot, and it is what the preset groups are designed around. Leave `textColor` alone, or set it to white, and do the same for the outline and shadow unless the preset already carries them. Use another colour only when the user asks for one — naming a colour, or naming a palette the video has to match. A colour you chose yourself, because it suited the shot or the brand you inferred, is the wrong kind of initiative here: it is the user's video, the choice is visible in every frame it covers, and a title that has gone unreadable over one bright shot is worse than a plain one. If you think a colour would genuinely serve the edit, say so and let them decide rather than applying it. The same holds for the letterform: `fontFamily` stays in the sans-serif family — `sans`, `geometric`, `rounded` or `impact` — never `serif`, `slab`, `mono` or `handwritten`, and `fontWeight` is 700 or above. A serif at a normal weight is thin, and thin white type over moving footage is the first thing to disappear. Anything else, again, only when the user asks for it.
