...
1. Call `get_editor_capabilities`. Before importing, look for a saved `.sve.json` in the media directory, preferring `simplevlogeditor-recovery.sve.json`; open and relink it first, then import only genuinely missing assets. For a new project, use `queue_media_import` with absolute paths, poll `get_import_status`, and never transfer media bytes through MCP.
