...
13. Give every tag, and every caption from the `background` group, more than four seconds on screen; below that they read as a flicker rather than as a message, and a Subscribe badge or QR code nobody had time to act on is worse than none. Ordinary `classic` subtitles are exempt — they follow the speech. Add captions, transitions, text cards, Subscribe tags, QR Code tags, audio changes, and output settings only when the brief asks for them or they clearly support it. Validate QR URLs and avoid covering important faces or screen content. Give text cards fade-in/out treatment; use transitions within one topic and a fade-out/fade-in break when the topic changes.
    - **A background caption is white, bold and sans serif.** White is what stays legible over footage that changes shade from shot to shot, and it is what the preset groups are designed around. Leave `textColor` alone, or set it to white, and do the same for the outline and shadow unless the preset already carries them. Use another colour only when the user asks for one — naming a colour, or a palette the video has to match. A colour you chose yourself, because it suited the shot or a brand you inferred, is the wrong kind of initiative: it is the user's video, the choice is visible in every frame it covers, and a title gone unreadable over one bright shot is worse than a plain one. Suggest it and let them decide rather than applying it. The letterform follows the same rule: `fontFamily` stays sans-serif — `sans`, `geometric`, `rounded` or `impact`, never `serif`, `slab`, `mono` or `handwritten` — and `fontWeight` is 700 or above, because thin white type over moving footage is the first thing to disappear.
