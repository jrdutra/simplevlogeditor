...
6. Transcribe with Small Quality and automatic spoken-language detection by default. When the prompt language is likely to match the recording, pass that language; ISO aliases such as `pt` are accepted. Use Voice/GTCRN with Balanced denoise when cleanup helps. Retry with Turbo only when `quality.reviewRecommended` or visibly incoherent text justifies it. Give realistic whole-operation and per-stage timeouts.
7. Use source timestamps for trims, splits, deleted ranges, captions, and push-ins. Use the output timing returned by `get_timeline` only when reasoning about the assembled result.
8. Preserve context, natural speech rhythm, reactions, intentional pauses, and important visual actions. Remove failed takes, redundant repetitions, errors, long accidental pauses, and off-topic material only when supported by the transcript, audio analysis, or frames.
9. Cut the mistake and keep the correction — that is the ordinary work of an edit, not an optional flourish. Someone talking to camera without a script gets things wrong and says them again, and the transcript's word timings show where. Recognise: a **restart** (the sentence begun, abandoned, begun again — cut from the abandoned attempt to the good one); a **correction** (said wrong then immediately fixed — cut the wrong value and the repair phrase); **groping for the word** (seconds of "the, uh, the thing that…" before the sentence arrives — cut the groping); a **stumble** (a word mangled then repeated cleanly); and **the same point made twice**, where the better one is kept wherever it sits. Cut on the speech rather than the clock: place boundaries on word timings, use `analyze_silence` to find the breath the join can hide in, start at the end of the last good word and end at the start of the first good word, and never cut inside a word. A pause for effect, thinking aloud that arrives somewhere, emphasis by repetition, a reaction or a stammer that is simply how the person talks are **not** mistakes — cutting every hesitation produces a different speaker, not a better take. When it could be either, leave it and say so. Report each of these cuts: where, what went, and the words that now meet.
10. Use `apply_edit_batch` with the latest `projectRevision` and `dryRun: true` before every subjective or multi-operation change. Inspect the result, then commit the same batch. Keep related edits in one batch so the user can undo them together.
11. Re-read the timeline after each committed batch. Use `preview` and inspect frames around important joins before declaring the edit finished.
12. Use `add_push_in` only when emphasis is editorially justified. Prefer a restrained scale and smooth ramp. Push-in times are source times and remain aligned through cuts and speed changes.
13. Give every tag, and every caption from the `background` group, more than four seconds on screen; below that they read as a flicker rather than as a message, and a Subscribe badge or QR code nobody had time to act on is worse than none. Ordinary `classic` subtitles are exempt — they follow the speech. Add captions, transitions, text cards, Subscribe tags, QR Code tags, audio changes, and output settings only when the brief asks for them or they clearly support it. Validate QR URLs and avoid covering important faces or screen content. Give text cards fade-in/out treatment; use transitions within one topic and a fade-out/fade-in break when the topic changes.
    - **A background caption is white, upper case, bold and sans serif.** White is what stays legible over footage that changes shade from shot to shot, and it is what the preset groups are designed around. Leave `textColor` alone, or set it to white, and do the same for the outline and shadow unless the preset already carries them. Use another colour only when the user asks for one — naming a colour, or a palette the video has to match. A colour you chose yourself, because it suited the shot or a brand you inferred, is the wrong kind of initiative: it is the user's video, the choice is visible in every frame it covers, and a title gone unreadable over one bright shot is worse than a plain one. Suggest it and let them decide rather than applying it. The letterform follows the same rule: `fontFamily` stays sans-serif — `sans`, `geometric`, `rounded` or `impact`, never `serif`, `slab`, `mono` or `handwritten` — and `fontWeight` is 700 or above, because thin white type over moving footage is the first thing to disappear. It is upper case too: leave `uppercase` alone, which is the editor's own default, or set it `true`, never `false`. Capitals give a short title the weight to hold a frame it shares with a moving person and keep the text an even height, so it sits behind them without descenders cutting in.
...
14. Never invent clip IDs, push-in IDs, caption IDs, tag shapes, transition names, presets, or timing. Obtain them from capabilities, the timeline, transcription, or tool results.
15. Save or export only to a path admitted by the current MCP session. Do not overwrite an important file unless the user's brief explicitly names that destination. Exports accept a `requestId`; poll `get_operation_status` and use `cancel_operation` if the user stops a long render.
16. Never add something and then update it to style it. An id is assigned by the editor and is not knowable until the operation has run, so a guessed one is how a batch comes to be rolled back for a caption that was never created. `add_caption`, `add_image` and `add_video_effect` take the full style in the same operation. When a later operation genuinely needs an id an earlier one produced, read the batch result's `created` array — one entry per operation, in order — and note that a dry run assigns the same ids the commit will. Captions sit end to end on a container: `add_caption` fails with `no_room` when the ones already there reach its end, which is a reason to update the caption that is there, not to retry.
17. A path the editor may not reach raises a permission request in the editor window rather than an immediate failure: the user is shown what was asked for and picks the folder in their system's file dialog. Wait for that. If they decline, the call fails with `path_consent_denied` — a decision, not a fault. Report it as one, name the folder that was refused, and do not retry it, copy media elsewhere to get past it, or ask again for the same folder in the same session. `get_diagnostics` and `health_check` both report `roots`, `rootSource` and a readable `rootNotice`; read those before describing what the editor can and cannot see. `SVE_MCP_ROOTS` is an advanced override, not the usual way in — the usual way is the user allowing a folder, from the request itself or from Allowed folders in the editor's project settings.
